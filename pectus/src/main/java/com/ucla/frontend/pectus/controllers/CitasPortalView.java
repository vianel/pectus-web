...
public class CitasPortalView implements Serializable{
	
	@Wire
	private Window modalAsignarCita;
...
//	NUEVOS METODOS
	...
	public void inicializarAtributos() throws Exception{
		...
		public Window getModalAsignarCita() {
			return modalAsignarCita;
		}

		public void setModalAsignarCita(Window modalAsignarCita) {
			this.modalAsignarCita = modalAsignarCita;
		}
...
//	METODOS NUEVOS PARA AGREGAR Y EDITAR
	...
	public void asignarCitaPaciente(){
		...
		if (visita.getFecha() == null) {
			Clients.showNotification("DEBE ELEGIR UNA FECHA PARA LA CITA", true);
			visible = true;
		}else{
		...
		if (resp.equalsIgnoreCase("true") && resp2.equalsIgnoreCase("true"))
	      {
	  		Clients.showNotification("CITA ASIGNADA EXITOSAMENTE", true);
